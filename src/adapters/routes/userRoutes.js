...
module.exports = ({ authService, tokenBlacklist }) => {
    ...
    router.get('/users', authMiddleware(authService, tokenBlacklist), requirePermission('CAN_VIEW_USERS'), (req, res) => userController.getAllUsers(req, res));

    router.get('/user/:id', authMiddleware(authService, tokenBlacklist), requirePermission('CAN_VIEW_USERS'), validateGetUserId, (req, res) => userController.getUserById(req, res));

    router.get('/user/username/:username', authMiddleware(authService, tokenBlacklist), requirePermission('CAN_VIEW_USERS'), validateGetUserUsername, (req, res) => userController.getUserByUsername(req, res));

    router.put('/user/:id', authMiddleware(authService, tokenBlacklist), requirePermission('CAN_UPDATE_USERS'), validateUserUpdate, (req, res) => userController.updateUser(req, res));
